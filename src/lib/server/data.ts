...
function deepMerge(
	...
	for (const key of Object.keys(incoming)) {
		...
		if (Array.isArray(currentValue) && Array.isArray(incomingValue)) {
			const currentArr = currentValue as Record<string, unknown>[];
			const incomingArr = incomingValue as Record<string, unknown>[];
			if (currentArr[0]?.id !== undefined || incomingArr[0]?.id !== undefined) {
				// merge by id — update existing items, append new ones
				const currentMap = new Map(currentArr.map((item) => [item.id, item]));
				for (const item of incomingArr) {
					currentMap.set(item.id, { ...(currentMap.get(item.id) ?? {}), ...item });
				}
				result[key] = Array.from(currentMap.values());
			} else {
				result[key] = [...new Set([...currentArr, ...incomingArr])];
			}
		} else if (
 ...
 */
export async function removeData(locals: App.Locals, toRemove: Record<string, unknown>) {
	const userId = locals.user?.id;
	if (!userId) throw new Error('User not authenticated');
...
function deepRemove(
	...
	for (const key of Object.keys(toRemove)) {
		...
		if (removeVal === null) {
			...
			typeof removeVal === 'object' &&
			!Array.isArray(removeVal) &&
...
export async function updateWidget(
	...
	const row = await db.select().from(data).where(eq(data.userId, userId)).limit(1);
	const currentData = (row[0]?.data ?? {}) as Record<string, unknown>;
	const widgets = (currentData.widgets ?? []) as Record<string, unknown>[];

	const updatedWidgets = widgets.map((w) => (w.id === widgetId ? { ...w, ...updates } : w));

	const updatedData = { ...currentData, widgets: updatedWidgets };
	await db.update(data).set({ data: updatedData }).where(eq(data.userId, userId));
	return updatedData;
}
